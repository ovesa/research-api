...
async def get_most_cited_in_collection(limit: int = 20) -> list[dict]:
    """Find the most-cited papers within your collection. Counts how many 
    times each paper is cited by other papers already in your database. 
    Papers cited by many other papers in your collection are likely 
    foundational to your research area.

    Args:
        limit: Number of top papers to return. Defaults to 20.

    Returns:
        List of dicts with paper metadata and an internal_citation_count field
            showing how many papers in your database cite it.
    """
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            ...
            JOIN papers p ON p.identifier = r.cited_identifier
            GROUP BY
                p.identifier, p.title, p.authors,
