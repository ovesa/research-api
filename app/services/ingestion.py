...
class IngestionResult:
    """Summary of a bulk ingestion run. Returned after an ingestion
    job completes so the caller knows exactly what happened (e.g.,
    how many papers were found, how many passed validation, how many
    were already in the database, and how many failed).

    Attributes:
        total_found (int): Total papers returned by the arXiv search.
        already_stored (int): Papers skipped because they were already
                                in Postgres. Not refetched or revalidated.
...
async def _search_arxiv_category(
    ...
    """Search arXiv for recent papers in a given category. Uses the arXiv API
    search endpoint to find recent submissions. Results are sorted by submission
    date descending so the most recent papers appear first.

    Args:
        category (str): The arXiv category to search. e.g. 'astro-ph.SR', 'physics.space-ph'
        max_results (int): Maximum number of results to return. Capped at 25
                            per category to avoid rate limiting. Defaults to 25.

    Returns:
        list[str]: List of arXiv IDs found in this category. Returns empty list on
...
async def _search_arxiv_date_range(
    ...
    """Search arXiv for papers in a category within a date range. Uses the
    arXiv API submittedDate filter to find papers submitted between two dates.
...
async def _search_ads(
    ...
    """Search NASA ADS for heliophysics papers within a date range. Uses
    the ADS search API to find papers published in heliophysics journals
    between two dates. ADS is better than arXiv for finding published papers
    because it indexes all major journals directly.
...
async def ingest_latest_heliophysics(
    ...
    Args:
        max_per_category (int): Maximum papers to fetch per arXiv category.
                                    Defaults to 25. Total papers fetched will
                                    be at most max_per_category * number of
                                    categories, minus duplicates and already
                                    stored papers.

    Returns:
        IngestionResult: Summary of what was found, skipped, ingested, rejected,
                            and failed during this run.
    """
...
async def _process_arxiv_ids(
    ...
    """Fetch, validate and save a list of arXiv IDs. Shared processing
    logic used by both ingest_latest_heliophysics and ingest_date_range.
    Checks Postgres before fetching, validates heliophysics relevance,
    and saves passing papers.
    ...
    Returns:
        IngestionResult: Summary of what was ingested, skipped, rejected,
                            and failed.
    """
    result = IngestionResult(
        total_found=len(all_ids),
...
async def ingest_date_range(
    ...
    """Ingest heliophysics papers from arXiv within a date range. Searches
    all heliophysics categories for papers submitted between the given dates.
    Useful for backfilling historical data.
...
async def ingest_by_ids(arxiv_ids: list[str]) -> IngestionResult:
    """Fetch and store a specific list of arXiv papers. Used when
    you have a known list of arXiv IDs to ingest rather than searching
    by category. Skips papers already in Postgres.
